    ...
    public class OrdersController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetOrderById(Guid id)
        {
            ...
            var response = new HALResponse(result)
                            .AddLinks(new Link("self", $"/core/v1/orders/{id}"))
                            .AddLinks(new Link("token", "/core/v1/orders/links/{Token}"));
            return Ok(response);
        }
        ...
        public async Task<IActionResult> GetOrderDetails(String token)
        {
            ...
            var response = new HALResponse(result)
                             .AddLinks(new Link("self", $"/core/v1/orders/links/{token}"))
                             .AddLinks(new Link("order", "/core/v1/orders/{OrderId}"))
                             .AddLinks(new Link("user", "/core/v1/users/{AttendeeId}"))
                             .AddLinks(new Link("concert", "/core/v1/concerts/{EventId}"));
        ...
        public async Task<IActionResult> GetOrders([FromQuery(Name = "status")]string status = "")
        {
            ...
            var response = new HALResponse(null)
                            .AddLinks(new Link("self", $"/core/v1/orders"))
                            .AddEmbeddedCollection("user", results,
                                new Link[] {
                                    new Link("self", "/core/v1/orders/{OrderId}")
                                });
            return Ok(response);
        }
